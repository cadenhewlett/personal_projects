...
error <- function(xt, zt, yt, params, configs) {
  ...
  if (any(!is.finite(preds$mu_hat)) | any(is.na(preds$mu_hat))) {
    ...
    if (configs$p > 0){
      # get AR params
      phi <- params[(4*configs$k + 1):(4*configs$k + configs$p)] 
...
Noting that we include additional checks for stationarity and invertibility of the ARMA process. In theory, this would allow us to have an unconstrained optimization on $\boldsymbol{\phi}$ and $\boldsymbol{\theta}$.
