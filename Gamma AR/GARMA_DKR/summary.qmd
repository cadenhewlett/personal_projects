...
- OLS log-likelihood and error structure implies that $y_t$ is conditionally normal, while the data itself is non-negative with a strong right tail.
...
- <span class="fragment">Model assumes that peaks (flood seasons) and troughs (dry seasons) follow the same process - i.e., no thresholding is modeled. </span>
:::
...
::: {style="font-size:60%; line-height:1.2;"}
We have, to this point, assumed a Gamma likelihood on the response $\mathbf{y}_T$ due to the non-negativity of the response (which was not accounted for in previous modelling procedures.) In practice, the heavy tail of the response was not completely captured by a Gamma likelihood. Hence, I considered a **generalized Gamma** distribution ([link](https://en.wikipedia.org/wiki/Generalized_gamma_distribution) on $\mathbf{y}_T$, i.e.
$$
f_Y(y_t; \theta, \alpha, \tau) = \dfrac{1}{\Gamma(\alpha / \tau)}\left(\dfrac{\tau}{\theta^\alpha}\right)y_t^{\alpha-1}e^{-(y_t/\theta)^\tau}
$$
Where $\theta$ is the scale and $\alpha, \kappa$ are shape parameters. If $\tau = 1$ the above is a regular Gamma distribution.

The mean-paramaterization of the Generalized Gamma is rather direct, as
$$
\mathbb{E}(y_t) = \mu_t = \theta \dfrac{\Gamma( (\alpha+1)/\tau )}{\Gamma(\alpha / \tau)}, \text { thus } \theta = \mu_t  \dfrac{\Gamma(\alpha / \tau)}{ \Gamma( (\alpha+1)/\tau )} = \mu_t\, \text{M}
$$
Where $\text{M}$ is a constant function of the shape parameters.
:::

###  Generalized Gamma Likelihood (ctd.)

::: {style="font-size:60%; line-height:1.2;"}
From the above, the mean-paramaterized Generalized gamma likelihood is given as
$$
f_Y(y_t; \mu_t, \alpha, \tau) =  \dfrac{1}{\Gamma(\alpha / \tau)}\left(\dfrac{\tau}{(\mu_t\, \text{M}({\alpha, \tau}))^\alpha}\right)y_t^{\alpha-1}e^{-(y_t/\mu_t\, \text{M}({\alpha, \tau}))^\tau}
$$

With log-density given by
$$
\log\,f_Y(y_t; \mu_t, \alpha, \tau) = \alpha \Big( \log(y_t)-\log (\mu_t) - \log(\text{M})\Big) - \tau \Big( \frac{y_t}{\mu_t \, \text{M}} \Big) - \log(\Gamma(\alpha / \tau)) + \log(\tau), \text { where } \text{M} = \dfrac{\Gamma(\alpha / \tau)}{ \Gamma( (\alpha+1)/\tau )}
$$
...
### Composite Loss Rather than Pure MLE

::: {style="font-size:60%; line-height:1.2;"}
Under a Generalized Gamma likelihood, the extremes of the data are more permissible. However, due to their rarity and the size of the dataset under pure Maximum Likelihood they are unlikely to be fitted correctly. Consequently, I added a least-squares penalty to the objective function. Letting $\boldsymbol{\Xi} = \{\boldsymbol{\delta}, \boldsymbol{\sigma}, \boldsymbol{\beta}, \boldsymbol{\varphi}, \boldsymbol{\theta}\}$ be the parameter set, 
$$
\text{Error}(\boldsymbol{\Xi} \mid \mathcal{F}_{t}) = \ell(\boldsymbol{\Xi}) - \lambda \sum_{t \in T} (y_t - \mu_t)^2
$$
Where $\lambda$ is a hyper-parameter to be estimated by grid search or CV. In this instance, our estimated set $\widehat{\boldsymbol{\Xi}}$ is a pseudo-true parameter set that minimizes the expected composite loss.  While it is plausible that an IRLS procedure would function similarly (and is valid under our model given fixed kernel parameters), the base `glm` package doesn't support generalized gamma distributions so we will need to write our own. 
:::

### Dunn-Smyth Residuals

::: {style="font-size:70%; line-height:1.2;"}
Since the likelihood on the response is no longer of a simple exponential family, I decided to use Dunn-Smyth residuals instead of Pearson and Deviance. Dunn-Smyth residuals are found by finding the corresponding normal deviate for each residual, i.e.
$$
r_{q, i} = \Phi^{-1}\{F(y_i ; \hat{\mu}_i, \hat{\alpha}, \hat{\tau})\}
$$
Where $\Phi(\cdot)$ is the CDF of the Standard normal and $F(\cdot)$ is the CDF of the Generalized Gamma distribution. If the model parameters are consistently estimated, $r_{q,i}$ converges to the standard normal distribution [(source)](https://gksmyth.github.io/pubs/residual.pdf). 

In our case, the CDF is given by
$$
F(y_t ; {\mu}_i, {\alpha}, {\tau}) = \frac{1}{\Gamma(\alpha / \tau)} \; \gamma \Big(\frac{\alpha}{\tau}, \Big( \frac{y_t}{\mu_t \, \text{M}}\Big)^{\tau} \Big)
$$
Where $\gamma(\cdot)$ is the lower incomplete gamma function, and $\text{M}$ is the previously-defined constant function of $\alpha$ and $\tau$.
```{r}
# generalized gamma density with mean parameterization
pggamma_mean <- function(y, mu, alpha, tau, log = FALSE) {
  # compute const
  M <- gamma(alpha/tau) / gamma( (alpha + 1)/tau )
  const <- mu*M
  # below is equivalent to pgamma(y/const)^tau, alpha/tau, scale=1
  unname( gammainc( (y/const)^tau, (alpha/tau) )["reginc"] )
}
```
:::
...
:::{style="font-size:70%; line-height:1.2;"}
For our experiment, we take the updated likelihood and allow $\mathbf{X} = \begin{bmatrix} \mathbf{1} & \mathbf{x}_T & \mathbf{x}_T & \mathbf{x}_T & \mathbf{x}_T\mathbf{z}_T\end{bmatrix}$. Each $\boldsymbol{\kappa}^{(i)}$ is Gamma-distributed, except for $\boldsymbol{\kappa}^{(1)}$ which is a 'fixed' kernel $\boldsymbol{\kappa}^{(1)} = \langle 1, 0, \dots, 0 \rangle$. This allows for the first coefficient $\beta_1$ to act as an intercept term. For this fit, we allow $p=q=0$. The 'classical' GLM framework doesn't apply for the generalized gamma, so we don't need to worry about a link structure. 

A summary of the estimated parameters is included below. 
```{r, echo = FALSE}
source(here::here("Gamma AR", "GARMA_DKR", "garma_dkr_main.R"))
mod <- readRDS(here::here("Gamma AR", "GARMA_DKR", "koksilah_ggamma_noar.RData"))
configs <- mod$configs
k <- configs$k - sum(configs$kernel_type == "fixed")
kernel_pars <- data.frame(matrix(mod$pars[1:(3 * k)], nrow = configs$k, byrow = TRUE))
kernel_pars <- cbind(kernel_pars, var = c("intercept", "x", "x", "x", "xz"))
colnames(kernel_pars) <- c("beta", "logdelta", "logsigma", "var")
kernel_pars
built_kernels2 <- sapply(1:configs$k, function(i) {
  build_kernel(kernel_pars$logdelta[i],
               kernel_pars$logsigma[i],
               type = configs$kernel_type[i])
})
kpars_out <- kernel_pars
kpars_out$scale <- exp(kernel_pars$logdelta)
kpars_out$shape <- exp(kernel_pars$logsigma)
kpars_out$delta <- kpars_out$shape*kpars_out$scale
kpars_out$sigma <- kpars_out$shape*(kpars_out$scale^2)
kpars_out <- cbind(kpars_out[-1, ], covariate = c("x", "x", "x", "xz"))

kable(
  (kpars_out[, c("scale", "shape", "beta", "delta", "sigma", "covariate")][
  order(kpars_out[, "delta"]), ]),
    caption = "Coefficient Estimates: Koksilah River",
  col.names =  c("Scale", "Shape",  "$\\hat{\\beta}$", "$\\hat{\\delta}$", "$\\hat{\\sigma}$",  "Covariate"),
  digits = 2,
  escape = FALSE
)
```
...
### Kernel Plots and Weighted $\mathbf{x}_T$ Contributions

```{r, echo = FALSE, fig.width=10}
# covariate labels in the same order as the kernel columns you keep
covars <- c("x","x","x","xz")   # length must equal number of K* columns

ell <- max(sapply(built_kernels2, length))
kernel_df2 <- as.data.frame(sapply(built_kernels2, function(x) c(x, rep(0, ell - length(x)))))

kernel_df2 <- kernel_df2[, -1]  # if you drop a column (e.g., intercept), reflect that in covars
colnames(kernel_df2) <- paste0("K", seq_len(ncol(kernel_df2)))
stopifnot(length(covars) == ncol(kernel_df2))  # safety check

kernel_df2$idx <- 0:-(nrow(kernel_df2)-1)

kernel_long2 <- tidyr::pivot_longer(
  kernel_df2,
  cols = -idx,
  names_to = "Kernel",
  values_to = "Value"
...
lookup <- tibble::tibble(
  Kernel = paste0("K", seq_along(covars)),
  Covariate = covars
)

kernel_long2 <- dplyr::left_join(kernel_long2, lookup, by = "Kernel")

# Plot: color by Covariate, group by Kernel so each kernel is a distinct line
pgg1 <- ggplot(kernel_long2, aes(x = idx, y = Value, group = Kernel, colour = Covariate)) +
  geom_line(linewidth = 0.5) +
  geom_point(size = 0.75) +
  labs(x = "Index", y = "Value", colour = "Covariate",
        subtitle = "Colour Indicates Covariates",
       title = "Unweighted Kernel Plots: Koksilah River") +
  theme_bw() +
...
# weighting
weights <- kpars_out$beta[1:4]
weighted_kern <- rowSums(sapply(1:(ncol(kernel_df2)-1), function(i){
  kernel_df2[, i]*weights[i]
}))
weighted_kern <- data.frame( weighted_kern / sum(weighted_kern))
weighted_kern <- cbind(weighted_kern, 0:(-1*(nrow(weighted_kern)-1)))
colnames(weighted_kern) <- c("weighted_kern", "lag")
pgg2 <- ggplot(weighted_kern, aes(y = weighted_kern, x = lag)) +
  geom_line(linewidth = 0.5) +
  geom_point(size = 0.75) +
  labs(x = "Lag", y = "Weighted Kernel", colour = "Kernel",
       subtitle = "Weights Include Interaction Term",
       title = "Weighted Kernel Plot: Koksilah River") +
  theme_bw() + scale_color_d3("category10")

gridExtra::grid.arrange(pgg1, pgg2, ncol = 2)
```

### Test Set Performance Metrics

:::{style="font-size:70%; line-height:1.2;"}
Below, I include the test set diagnostics for the fitted model on the $\approx 10$ years of test data.
```{r, echo = FALSE}
yt_test <- mod$yt_test
mu_test <- mod$mu_test

y_bar <- mean(yt_test, na.rm = TRUE)
n <- sum(!is.na(yt_test))
RSS <- sum((yt_test - mu_test)^2, na.rm = TRUE)
TSS <- sum((yt_test - mean(yt_test, na.rm = TRUE))^2, na.rm = TRUE)
res <- data.frame(
  RMSE = sqrt(RSS / n),
  NRMSE = sqrt(RSS / n) / y_bar,
  R2 = 1 - RSS / TSS,
  NSE = hydroGOF::NSE(sim = mu_test, obs = yt_test),
  KGE = hydroGOF::KGE(sim = mu_test, obs = yt_test)
)
kable(
  res,
  digits = 2,
  caption = "Test Set Performance Metrics: No ARMA Error Term"
)
```

:::

### Dunn-Smyth Residuals and Test Set Fits
...
p1 <- ggplot(data.frame(rQ), aes(x = rQ)) +
  geom_histogram(bins = 40, fill = "grey70", color = "black") +
  labs(title = "Dunn–Smyth residuals", x = "", y = "Count") +
  theme_bw()

# 2. Normal Q-Q plot
p2 <- ggplot(data.frame(sample = rQ), aes(sample = sample)) +
  stat_qq(alpha = 0.15) +
    geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "Normal Q-Q Plot", x = "Theoretical Quantiles", y = "Sample Quantiles") +
  theme_bw()

# 3. Actual vs fitted (subset indices)
df3 <- data.frame(
  idx = 100:650,
  actual = yt_test[100:650],
  fitted = mu_test[100:650]
)
p3 <- ggplot(df3, aes(x = idx)) +
  geom_line(aes(y = actual), color = "black") +
  geom_line(aes(y = fitted), color = "red") +
  labs(title = "Actual (Black) vs. Fitted Mean (Red)", x = "Index", y = "Value") +
  theme_bw()

# 4. Residuals vs fitted mean
df4 <- data.frame(fitted = mu_test, residual = rQ)
p4 <- ggplot(df4, aes(x = fitted, y = residual)) +
  geom_point(size = 0.6, alpha = 0.25) +
  geom_abline(intercept = 0, slope = 0, color = "red") +  # horizontal line at 0
  labs(title = "rQ vs fitted mean", x = "Fitted mean", y = "Residual") +
  theme_bw()
