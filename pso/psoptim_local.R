psoptim_local <- function (par,
  ...
  if (p.type == 0) {
    ...
    V <- matrix(runif(
      npar * p.s,
      min = as.vector(lower - X),
      max = as.vector(upper - X)
    ), npar, p.s)
  ...
  while (stats.iter < p.maxit &&
    ...
    if (!p.vectorize) {
      ...
      if (p.type == 0) {
        ...
        temp <- j != (1:p.s)
        V[, temp] <- V[, temp] + mrunif(npar, sum(temp), 0, p.c.p) * (P[, j[temp]] -
                                                                        X[, temp])
      } else {
        # SPSO 2011
        temp <- j == (1:p.s)
...
set.seed(42)
print(pso::psoptim(rep(NA, 2), function(x){sum(x^2)},
                   lower = rep(-5, times = 10), upper = rep(5, times = 10),
                   control =list(s = 40))$value)
print(psoptim_local(rep(NA, 2), function(x){sum(x^2)},
